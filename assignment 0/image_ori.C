...
void WriteByte(FILE* file, unsigned char b) {
    int success = fwrite((char*)&b, sizeof(unsigned char), 1, file);
    //assert(success == 1);
}
...
void Image::SaveTGA(const char* filename) const {
    ...
    //std::cout<<"before save open alive!"<<std::endl;
    FILE* file = fopen(filename, "wb");
    std::cout<<"after fopen alive!"<<std::endl;

    // misc header information

    //char header[18] = {0,0,2,0,0,0,0,0,0,0,0,0,width % 256,width/256,height%256,height/256,24,32};
    //fwrite(header,1,19,file);
    std::cout<<"header part alive!"<<std::endl;
    // the data
